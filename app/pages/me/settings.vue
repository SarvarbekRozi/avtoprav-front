  ...
  <div class="max-w-2xl mx-auto px-4 sm:px-6 py-10" v-if="auth.user">
    ...
    <div class="space-y-5">
      ...
      <div class="card p-6">
        ...
        <!-- Taklif va murojaat — to'g'ridan-to'g'ri admin, muloyim/bilinar-bilinmas -->
        <a href="https://t.me/avtoprav_admin" target="_blank" rel="noopener"
           class="flex items-center gap-3 -mx-2 px-2 py-2 rounded-xl transition-colors hover:bg-ink-50 group">
          <span class="w-10 h-10 rounded-xl grid place-items-center flex-shrink-0" style="background: var(--surface-inset); color: var(--text-3);">
            <AppIcon name="user" :size="18" />
          </span>
          <div class="min-w-0 flex-1">
            <div class="text-sm font-semibold" style="color: var(--text-2);">@avtoprav_admin</div>
            <div class="text-xs mt-0.5" style="color: var(--text-3);">
              {{ i18n.t({ uz: 'Taklif va murojaatlar uchun', kr: 'Таклиф ва мурожаатлар учун' }) }}
            </div>
          </div>
          <AppIcon name="chev-r" :size="16" class="text-ink-300 flex-shrink-0 group-hover:translate-x-0.5 transition-transform" />
        </a>
